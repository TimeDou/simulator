...
parser Cmdline_Parser(int argc, char * argv[])
{
  ...
  option.add <string> ("mode", 'm', "Error Metric", false, "mred");
  option.add <int> ("num", 'n', "num. of testing patterns", false, 17);
...
int main(int argc, char * argv[])
{
  ...
  if (mode_str == "er")
    ...
    mode = 1;
  else if (mode_str == "mred")
    mode = 2;
  else
    DASSERT(0);
  double* ErrorMetrics = MeasureErrorMetrics(pNtk1, pNtk2, 1 << nFrame, FALSE, mode);
  ofstream f_error;
