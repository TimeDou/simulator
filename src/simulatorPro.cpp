...
uint64_t Simulator_Pro_t::FullSimGenerator(uint64_t lRound, uint64_t iIteration) {
  uint64_t lBlock = 6;
  uint64_t lBits = (1 << lBlock) * (iIteration);
  if (lRound >= lBlock)
    if (lBits / (1 << lRound) % 2)
      return ~((uint64_t)0);
    else
    ...
    uint64_t nDup = 1 << (lBlock - lRound - 1);
    uint64_t tmp = 0;
    uint64_t val = (~((~((uint64_t)0)) << (1 << lRound))) << (1 << lRound);
    for (uint64_t _ = 0; _ < nDup; _++)
      tmp = (tmp << (1 << (lRound + 1))) + val;
    return tmp;
  }
}

void Simulator_Pro_t::Input(int iBatch) {
  // primary inputs
  Abc_Obj_t * pObj = nullptr;
  int k = 0;
  //cout << "Test input PIs" << endl;
  Abc_NtkForEachPi(pNtk, pObj, k) {
    for (int i = 0; i < nBlock; i++) {
      values[pObj->Id][i] = FullSimGenerator(k, iBatch*uint64_t(nBlock) + i);
      /*tmpValues[pObj->Id][i] = values[pObj->Id][i];*/
    }
  }
  //cout << "Test input PIs over!!!" << endl;
  // constant nodes
  //cout << "Test input constant nodes" << endl;
...
void Simulator_Pro_t::PrintInputStream(int frameId, bool isReverse) const
{
  Abc_Obj_t * pObj = nullptr;
  int i = 0;
  int blockId = frameId >> 6;
